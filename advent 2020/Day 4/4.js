...
const mapify = (passport) => {
  let items = passport.replace(/(?:\r\n|\r|\n)/g, " ").split(" ")
  let dic = {}
...
passports = passports.map(passport => mapify(passport));
console.log(passports.length);
validatePassport = (passport) => {
  if (Object.keys(passport).length === 8) {
    return true
  } else if (passport["cid"] == undefined && Object.keys(passport).length == 7) {
    return true
  } else {
    return false
  }
...
newValidation = (passport) => {
  if (Object.keys(passport).length < 7 || (Object.keys(passport).length === 7 && passport["cid"] !== undefined)) {
    console.log(Object.keys(passport).length+" passport invalid;not all fields are present");
    return false;
  } 
  if (!passport["byr"] || passport["byr"].length !== 4 || passport["byr"] < 1920 || passport["byr"] > 2002) {
    console.log(passport["byr"] + " invalid: byr");
    return false;
  } 
  if (!passport["iyr"] || passport["iyr"].length !== 4 || passport["iyr"] < 2010 || passport["iyr"] > 2020) {
    console.log(passport["iyr"] + " invalid iyr");
    return false; 
  }
  if (!passport["eyr"] || passport["eyr"].length !== 4 || passport["eyr"] < 2020 || passport["eyr"] > 2030) {
    console.log(passport["eyr"] + " invalid: eyr");
    return false;
  } 

  if (!passport["hgt"]) {
    console.log(passport["hgt"] + " invalid: hgt");
    return false;
  } else {
    let metric = passport["hgt"].slice(-2)
    let height = Number(passport["hgt"].slice(0, -2));
    if (!(metric === "in" || metric === "cm")) {
      console.log(passport + " invalid: hgt");
      return false;
    } 
  ...
  if (!passport["hcl"] || passport["hcl"][0] !== "#") {
    console.log(passport["hcl"] + " invalid: hcl");
    return false;
  } else {
    if (!(/^[0-9a-zA-Z]+$/.test(passport["hcl"].slice(1)))) {
      console.log(passport["hcl"] + " invalid: hcl");
      return false;
    } 
  }
  
  if (!passport["ecl"] || ["amb", "blu","brn", "grn", "hzl", "gry", "oth"].indexOf(passport["ecl"]) < 0) {
    console.log(passport["ecl"] + " invalid: ecl");
    return false;
  } 
  
  if (!passport["pid"] || passport["pid"].length !== 9) {
    console.log(passport["pid"] + " invalid: pid");
    return false;
  } 
